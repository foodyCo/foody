...
export default async function Profile({
  ...
  try {
    ...
    postsResult = Array.isArray(posts?.results || posts) ? (posts?.results || posts).map(mapDjangoPostToDish) : [];
    savedResult = Array.isArray(saved?.results || saved) ? (saved?.results || saved).map(mapDjangoPostToDish) : [];
    userProfile = me;
  } catch (e: any) {
  ...
  const bio = userProfile?.bio_text || "Здесь пока нет описания профиля...";
  const stats = {
        ...
        <section className="hide-scroll flex-1 overflow-y-auto px-4 pb-25">
          ...
          <div className="mt-5 grid grid-cols-2 rounded-full border border-white/60 bg-white/40 p-1 shadow-[inset_1px_1px_0_rgba(255,255,255,0.7)]">
            <Link
              ...
              prefetch={false}
              className={
                "py-2 text-center text-[13.5px] font-bold rounded-full transition-colors " +
                (activeTab === "posts"
